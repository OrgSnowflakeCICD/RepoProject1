...
USING SCHEMACHANGE.CHANGE_HISTORY_{TARGET_STAR_SCHEMA} source 
ON target.SCRIPT = source.SCRIPT AND target.TARGET_STAR_SCHEMA = '{TARGET_STAR_SCHEMA}' 
WHEN MATCHED AND (target.SCRIPT_TYPE = 'A' OR (target.SCRIPT_TYPE = 'R' AND target.CHECKSUM != source.CHECKSUM))
    ...
    ,target.TARGET_STAR_SCHEMA = '{TARGET_STAR_SCHEMA}'       
WHEN NOT MATCHED 
    THEN INSERT (
    VERSION
    ,DESCRIPTION
    ...
    DROP TABLE IF EXISTS SCHEMACHANGE.CHANGE_HISTORY_{TARGET_STAR_SCHEMA};
